...
def game_intro():  # loop for the intro page- click events for starting the game
    ...
    while intro:  # while loop for exit
        ...
        button("START", 200, 420, 100, 50, white, black, "play")  # for the first button
        button("QUIT", 200, 520, 100, 50, white, black, "quit")  # for the second button
        button("INSTRU",200, 620, 100, 50, white, black,"instructions")
...
def instructions(): #instructions page
    ...
    while intro:  # while loop for exit
        ...
        sText = pygame.font.Font('freesansbold.ttf', 40)  # places the word Space- color,height, width, font, size
        TextSurf, TextRect = text_objects("How to Play", sText)  # font and text
        TextRect.center = ((display_width / 2), (display_height / 6))  # placement
        gameDisplay.blit(TextSurf, TextRect)  # blit it on surface

        iText = pygame.font.Font('freesansbold.ttf', 20)  # places the word Invaders- color,height, width, font, size
        TextSurf, TextRect = text_objects("Move the ship using your arrow keys", iText)  # font and text
        TextRect.center = ((display_width / 2), (display_height / 3))  # placement
        gameDisplay.blit(TextSurf, TextRect)  # blit on surface

        iText = pygame.font.Font('freesansbold.ttf', 20)  # places the word Invaders- color,height, width, font, size
        TextSurf, TextRect = text_objects("Shoot the enemy ships using the space bar", iText)  # font and text
        TextRect.center = ((display_width / 2), (display_height / 2))  # placement
        gameDisplay.blit(TextSurf, TextRect)  # blit on surface

        iText = pygame.font.Font('freesansbold.ttf', 20)  # places the word Invaders- color,height, width, font, size
        TextSurf, TextRect = text_objects("Get as high a score as possible", iText)  # font and text
        TextRect.center = ((display_width / 2), (display_height / 1.5))  # placement
        gameDisplay.blit(TextSurf, TextRect)  # blit on surface
...
class controller:
    def __init__(self):
        ...
        while not gameexit:  # while loop of game events - with a for loop for keys for player
            ...
            if button("B", 20, 20, 30, 30, white, black, "back"):
                game_intro()
           ...
           # if button("P", 20, 55, 30, 30, white, black, "pause"):
            ...
            enemy.move()  # calling enemyships class
            enemy.move()
            enemy.move()
         ...
         #   if hits:
        #        gameexit = True
            # if hero.rect.bottom < enemy.rect.bottom or hits:  # creating boundary for crash
